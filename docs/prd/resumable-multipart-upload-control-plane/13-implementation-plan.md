...
## 30. Product Implementation Plan

This section is written for engineering execution. The product target is a production-oriented upload control plane, not a minimal demo. The implementation must still be split into reviewable, runnable stages so each stage has clear ownership, tests, and rollback shape.
