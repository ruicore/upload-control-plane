...
The repository can be considered ready for local evaluation when:
...
```text
This repository implements a production-oriented large-file upload control plane for research, media, and field-data ingestion. It uses S3-compatible multipart upload to allow clients to upload file parts directly to object storage while the backend controls authorization, presigned URL generation, session state, completion, abort, cleanup, and observability.

The design intentionally separates the control plane from the data plane: the API service and optional EMQX/MQTT adapter never receive large file bodies. This keeps backend bandwidth and memory usage bounded while allowing browsers, CLI tools, and field clients to upload large datasets directly to MinIO/S3 with resumability and parallelism.

The project also models the surrounding data-platform concerns: tenants, projects, datasets, upload tasks, device credentials, resource-scoped permissions, storage policies, validation, recycle/restore/purge lifecycle, audit events, and a transactional outbox for recoverable event delivery.
```
...
## 36. Recommended Second Implementation Task
...
```text
Production-grade large-file upload is not about making the backend better at receiving huge files.
