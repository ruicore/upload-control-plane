...
```text
UploadTask: field-capture-2026-06-10-site-west
  - camera-primary.mp4
  - camera-secondary.mp4
  - lidar.bin
  - sensor-state.jsonl
  - diagnostics.zip
```
...
Field terminals and remote clients must be first-class subjects, not only free-text device labels.
...
- `device_code` is the human or deployment-facing external code such as `field-client-17`; it may be unique per tenant but must not replace `device.id` as the internal identifier.
...
For research, media, and field data, extracted metadata may include:
...
Data platforms need explicit lifecycle governance.
