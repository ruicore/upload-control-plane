...
Research, media, and field-data workloads often generate large files:

- Camera and sensor logs.
- Video streams.
- LiDAR frames.
- Structured experiment archives.
- Research datasets.
- Edge-device diagnostic bundles.
- Field inspection images.
...
```text
media-upload-control-plane
resumable-upload-control-plane
field-data-upload-control-plane
```
...
Optional future extensions are allowed, but implementers must not add them before core upload correctness is complete.
