...
def test_domain_write_and_outbox_append_commit_atomically() -> None:
    ...
    with _session_scope(session_factory) as session:
        ...
        _delete_outbox_test_artifacts(session)
    ...
    try:
        with (
            ...
            _insert_dataset(session, dataset_id=dataset_id, name="ucp-test-outbox-atomic")
            append_outbox_event(
                session,
                _append(
                    event_id=event_id,
                    aggregate_id=dataset_id,
                    event_type="ucp.test.outbox.atomic",
                ),
            )
...
def test_delivery_failure_does_not_roll_back_committed_domain_state() -> None:
    ...
    with _session_scope(session_factory) as session:
        seed_dev_data(session, get_settings())
        _delete_outbox_test_artifacts(session)
        _insert_dataset(session, dataset_id=dataset_id, name="ucp-test-outbox-domain-survives")
        append_outbox_event(
            session,
            _append(
                event_id=event_id, aggregate_id=dataset_id, event_type="ucp.test.outbox.domain"
            ),
        )
...
def test_outbox_cleanup_preserves_unrelated_generic_outbox_prefixes() -> None:
    session_factory = _db_session_factory_or_skip()
    suffix = uuid.uuid4().hex
    owned_dataset_id = uuid.uuid4()
    owned_event_id = uuid.uuid4()
    unrelated_dataset_id = uuid.uuid4()
    unrelated_event_id = uuid.uuid4()
    ...
    with session_factory() as session:
        ...
        _insert_dataset(
            ...
            name=f"ucp-test-outbox-cleanup-counterexample-{suffix}",
        )
        append_outbox_event(
            session,
            _append(
                event_id=owned_event_id,
                aggregate_id=owned_dataset_id,
                event_type=f"ucp.test.outbox.cleanup-counterexample.{suffix}",
            ),
        )
        _insert_dataset(
            session,
            dataset_id=unrelated_dataset_id,
            name=f"outbox-unrelated-{suffix}",
        )
        session.flush()
        unrelated_dataset = session.get(Dataset, unrelated_dataset_id)
        assert unrelated_dataset is not None
        unrelated_object_key = f"outbox/unrelated-{suffix}.bin"
        unrelated_dataset.object_key = unrelated_object_key
        append_outbox_event(
            session,
            _append(
                event_id=unrelated_event_id,
                aggregate_id=unrelated_dataset_id,
                event_type=f"outbox.unrelated.{suffix}",
            ),
        )
        session.flush()
        ...
        assert session.get(Dataset, owned_dataset_id) is None
        assert session.get(OutboxEvent, owned_event_id) is None
        assert session.get(Dataset, unrelated_dataset_id) is not None
        assert session.get(OutboxEvent, unrelated_event_id) is not None
...
def _append(
    *,
    ...
    event_type: str = "ucp.test.outbox.test",
...
def _insert_dataset(session: Session, *, dataset_id: uuid.UUID, name: str) -> None:
    ...
    session.add(
        Dataset(
            ...
            object_key=f"ucp-test-outbox/{name}.bin",
            object_size_bytes=1,
...
def _delete_outbox_test_artifacts(session: Session) -> None:
    dataset_ids = list(
        session.scalars(select(Dataset.id).where(Dataset.name.like("ucp-test-outbox-%")))
    )
    session.execute(delete(OutboxEvent).where(OutboxEvent.event_type.like("ucp.test.outbox.%")))
    if dataset_ids:
